    ...
    const Submit = async (e) => {
        ...
        if (!newStatus) {
            ...
            try {
                if (!editingStatusId) {
<<<<<<< HEAD
                    const response = await axios.post('http://localhost:9000/Status', { newStatus });
=======
                    const response = await axios.post('http://localhost:9000/Status', { newStatus });
>>>>>>> d01921e03d8f442df2a15f95ec9e4853fc9f328d

                    if (response.status >= 200 && response.status < 300) {
                        enqueueSnackbar('Status Added Successfully!', { variant: 'success' });
                        setNewStatus('');
                        setStatuses([...statuses, response.data]);
                    } else {
                        enqueueSnackbar('Failed to add Status', { variant: 'error' });
                    }
                } else {
<<<<<<< HEAD
                    const response = await axios.put(`http://localhost:9000/Status/${editingStatusId}`, { newStatus });
=======
                    const response = await axios.put(`http://localhost:9000/Status/${editingStatusId}`, { newStatus });
>>>>>>> d01921e03d8f442df2a15f95ec9e4853fc9f328d
                    ...
                    if (response.status >= 200 && response.status < 300) {
                        ...
                        setStatuses(statuses.map(status => status.id === editingStatusId ? response.data : status));
                    } else {
...
>>>>>>> d01921e03d8f442df2a15f95ec9e4853fc9f328d
    ...
    const deleteStatus = async (id) => {
        try {
<<<<<<< HEAD
            const response = await axios.delete(`http://localhost:9000/Status/${id}`);
=======
            const response = await axios.delete(`http://localhost:9000/Status/${id}`);
>>>>>>> d01921e03d8f442df2a15f95ec9e4853fc9f328d
...
<<<<<<< HEAD
    ...
    const confirmDelete = async () => {
        if (sourceToDelete) {
            await deleteStatus(sourceToDelete);  // Call your existing delete function
        }
        closeDeleteModal();
    };
=======
...
>>>>>>> d01921e03d8f442df2a15f95ec9e4853fc9f328d
            ...
            <div className='fixed z-20 flex flex-col min-h-screen shadow-2xl overflow-hidden w-1/6 bg-black text-white' style={{ width: "15%" }}>
    ...
    {masters && (
        <div className='ml-3'>
            ...
            <h1 onClick={NavStatus} className='p-1 cursor-pointer bg-white mx-3 flex rounded-md mb-1 text-black'>
                <MdPending size={24} />
                &nbsp;Status
            </h1>
    ...
    <img className='p-4 mt-auto bg-white' src={loram} alt="Loram Logo" />
            ...
            <div className='ml-80 flex w-3/4 relative top-24'>
                <div className='mr-4 p-4 h-fit w-2/5 bg-white border border-gray-200 overflow-hidden shadow-lg'>
                    <h1 className='text-2xl mb-2 font-semibold'>{editingStatusId ? 'Edit Status' : 'Add Status'}</h1>
                    <hr />
                    <hr />
                    <br />
                    <div className='mx-4'>
                        <label className='block' htmlFor="Name">Status:</label>
                        <input className='border w-full border-gray-400 rounded-md p-2 overflow-hidden shadow-md outline-none' type="text" placeholder='Enter Status' value={newStatus} onChange={(e) => setNewStatus(e.target.value)}/>
                    </div>
                    <div className='mx-4 mt-8'>
                        <button onClick={Submit} className='mr-2 py-1 px-5 bg-blue-600 hover:bg-blue-900 overflow-hidden shadow-md text-white rounded-sm'>{editingStatusId ? 'Update' : 'Submit'}</button>
                ...
                <div className='ml-4 w-3/5 h-fit p-4 bg-white border border-gray-200 overflow-hidden shadow-lg'>
                    <h1 className='text-2xl mb-2 font-semibold'>Status</h1>
                    <hr />
                    <hr />
                    <br />
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className='bg-black text-white'>
                            <tr>
                                ...
                                <th className="border px-6 py-3 text-center tracking-wider text-sm font-medium uppercase">Status</th>
                        ...
                        <tbody className='bg-white divide-y divide-gray-200'>
                            {statuses.map((status, index) => (
                                <tr className='border' key={index}>
                                    <td className="px-6 text-center py-2">{status.id}</td>
                                    <td className="px-6 text-center py-2">{status.status}</td>
                                    <td onClick={() => editStatus(status.id, status)} className="px-3 text-center py-2 hover:bg-black hover:text-white cursor-pointer">
                                        <div className="flex justify-center items-center">
                                            <MdEditSquare size={20}/>
                                        </div>
                                    </td>
<<<<<<< HEAD
                                    <td onClick={() => openDeleteModal(status.id)} className="px-3 text-center py-2 hover:bg-black hover:text-white cursor-pointer">
=======
                                    <td onClick={() => deleteStatus(status.id)} className="px-3 text-center py-2 hover:bg-black hover:text-white cursor-pointer">
>>>>>>> d01921e03d8f442df2a15f95ec9e4853fc9f328d
                                        <div className="flex justify-center items-center">
                                            <FaTrash size={18}/>
                                        </div>
