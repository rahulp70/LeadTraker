...
<<<<<<< HEAD
                        const response = await axios.post('http://localhost:9000/leadTypes', { LeadType });
=======
                        const response = await axios.post('http://localhost:9000/leadTypes', { LeadType });
>>>>>>> d01921e03d8f442df2a15f95ec9e4853fc9f328d
                        if (response.status >= 200 && response.status < 300) {
                            ...
                            setLeadType('');
                            setLeadTypes([...LeadTypes, response.data]);
                        } else {
                            enqueueSnackbar('Please Fill Everything Correctly!', { variant: 'error' });
                        }
                    } else {
<<<<<<< HEAD
                        const response = await axios.put(`http://localhost:9000/leadTypes/${editingLeadTypeId}`, { LeadType });
=======
                        const response = await axios.put(`http://localhost:9000/leadTypes/${editingLeadTypeId}`, { LeadType });
>>>>>>> d01921e03d8f442df2a15f95ec9e4853fc9f328d
                        if (response.status >= 200 && response.status < 300) {
                            ...
                            setLeadTypes(LeadTypes.map(type => 
                                type.id === editingLeadTypeId ? response.data : type
                            ));
                        } else {
                            enqueueSnackbar('Failed to update Lead type', { variant: 'error' });
                        }
                    }
                } catch (error) {
...
>>>>>>> d01921e03d8f442df2a15f95ec9e4853fc9f328d
        ...
        const deleteLeadType = async (id) => {
            try {
<<<<<<< HEAD
                const response = await axios.delete(`http://localhost:9000/leadTypes/${id}`);
=======
                const response = await axios.delete(`http://localhost:9000/leadTypes/${id}`);
>>>>>>> d01921e03d8f442df2a15f95ec9e4853fc9f328d
                if (response.status >= 200 && response.status < 300) {
                    enqueueSnackbar('Lead Type Deleted Successfully!', { variant: 'success' });
                    setLeadTypes(LeadTypes.filter(type => type.id !== id));
                    setTimeout(() => {
                        window.location.reload();
                    }, 2000);
                } else {
                    enqueueSnackbar('Failed to delete Lead type', { variant: 'error' });
                }
            } catch (error) {
                console.error('Error:', error);
                enqueueSnackbar('Connection Error!', { variant: 'error' });
...
<<<<<<< HEAD
    ...
    const confirmDelete = async () => {
        if (leadTypetoDelete) {
            await deleteLeadType(leadTypetoDelete);  // Call your existing delete function
        }
        closeDeleteModal();
    };
...
>>>>>>> d01921e03d8f442df2a15f95ec9e4853fc9f328d
            <div className='fixed flex justify-end z-10 py-3 overflow-hidden w-full text-white '>
                ...
                <>
                    <div className='fixed text-black text-center px-2 w-1/12 shadow-2xl overflow-hidden bg-white border border-gray-400 mr-1 mt-10 z-50 justify-end rounded-lg'>
                        ...
                        <h1 onClick={logoutUser} className='flex px-6 py-2 rounded-md my-1 shadow-2xl overflow-hidden cursor-pointer hover:bg-black hover:text-white'><MdLogout size={24}/> Logout</h1>
                    </div>
                </>
                )}
            </div>
    ...
    {masters && (
        <div className='ml-3'>
            ...
            <h1 onClick={NavLeadType} className='p-1 cursor-pointer bg-white mx-3 flex rounded-md mb-1 text-black'>
                <MdCategory size={24} />
                &nbsp;Lead Type
            </h1>
            ...
            <div className='ml-80 flex w-3/4 relative top-24'>
                <div className='mr-4 p-4 h-fit w-2/5 bg-white border border-gray-200 overflow-hidden shadow-lg'>
                    ...
                    <div className='mx-4'>
                        ...
                        <input className='border w-full border-gray-400 rounded-md p-2 overflow-hidden shadow-md outline-none' type="text" placeholder='Enter Lead Type' value={LeadType} onChange={(e) => setLeadType(e.target.value)}/>
                    </div>
                    <div className='mx-4 mt-8'>
                    <button onClick={Submit} className='mr-2 py-1 px-5 bg-blue-600 hover:bg-blue-900 overflow-hidden shadow-md text-white rounded-sm'>{editingLeadTypeId ? 'Update' : 'Submit'}</button>
                        ...
                        <tbody className='bg-white divide-y divide-gray-200'>
                            {LeadTypes.map((LeadType, index) => (
                                <tr className='border' key={index}>
                                    <td className="px-6 text-center py-2">{LeadType.id}</td>
...
=======
                                    <td onClick={() => deleteLeadType(LeadType.id)} className="px-3 text-center py-2 hover:bg-black hover:text-white cursor-pointer">
>>>>>>> d01921e03d8f442df2a15f95ec9e4853fc9f328d
                                        <div className="flex justify-center items-center">
                                            <FaTrash size={18}/>
                                        </div>
